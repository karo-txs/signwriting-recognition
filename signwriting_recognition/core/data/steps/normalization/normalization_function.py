from core.utils.tf_data_functions import apply_transform_and_report
...
def dataset_landmark_normalization(tf_dataset, save_path, save_landmark_image=True):
    def normalize(hand_data):
        hand_data["hand_landmark"] = apply_all_normalizations(
            hand_data["hand_landmark"]
        )
        hand_data["world_hand"] = apply_all_normalizations(hand_data["world_hand"])
        return hand_data

    tf_dataset = apply_transform_and_report(
        tf_dataset,
        normalize,

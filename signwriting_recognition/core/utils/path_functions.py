...
def get_relation_of_files_per_folder_name(
    file_paths: list, folder_names: list, limit_value: int = None
):
    """
    ...
    index_by_folder = dict((name, index) for index, name in enumerate(folder_names))

    # Extract the folder name from each file path and map it to its corresponding index
    all_file_indices = [
        os.path.basename("/".join(path.split("/")[:-1])) for path in file_paths
    ]
    ...
    if limit_value:
        ...
        for origin_data_path, folder_name in zip(file_paths, all_file_indices):
            ...
            if (
                folder_name in class_occurrences.keys()
                and len(class_occurrences[folder_name]) >= limit_value
            ):
                limit_exceeded = True
...
def get_internal_asset(asset_name: str):
    with pkg_resources.path("infra.model_dependences", asset_name) as asset_path:
        return asset_path


def get_internal_asset_folder(asset_name: str):
    asset_path = pkg_resources.files("infra.model_dependences").joinpath(asset_name)
    return asset_path
